...
def setup_plotting_area(parent_frame, app_instance):
    ...
    app_instance.axs_plots[0].set_ylim(-np.pi - 0.5, np.pi + 0.5)  # Initial guess
    ...
    app_instance.axs_plots[1].set_ylim(-np.pi - 0.5, np.pi + 0.5)  # Initial guess
    ...
    app_instance.axs_plots[2].set_ylim(-15, 15)  # Initial guess
    ...
    app_instance.axs_plots[3].set_ylim(-15, 15)  # Initial guess

    app_instance.fig_plots.tight_layout()


def setup_control_panel(parent_frame, app_instance):
    """Creates the control panel widgets, including expanded PID tuning and torque display."""
    row_idx = 0
    main_colspan = 3
    ...
    param_vars_data = [('m1', 1.0), ('l1', 1.0), ('m2', 1.0), ('l2', 1.0)]
    ...
    control_methods = ["None", "PID", "LQR", "PPO RL Agent"]
    ...
    # This frame is managed by grid, its internal row_idx starts from 0 for its children
    ...
    pid_params_config = [
        ...
        ('kp_q2', "Kp (q2):", 'kp_q2_val', 'kp_q2_var', [1.0, 0.5, 0.1]),
    ...
    for key, label, val_attr, var_attr, incs in pid_params_config:
        if not hasattr(app_instance, var_attr):
            default_val = app_instance.DEFAULT_CONTROLLER_PARAMS['pid'].get(key.upper(), 0.0)  # Example: Kp_q1
        ...
        for inc in incs:
            # Use key (e.g., 'kp_q1') directly for adjust_pid_param
    ...
    tau_frame = ttk.Frame(parent_frame)
    tau_frame.grid(row=row_idx, column=0, columnspan=main_colspan, sticky="ew", pady=(5, 0), padx=5)
    ttk.Label(tau_frame, text="Current Torque (τ1):").pack(side=tk.LEFT, padx=(0, 5))
    if not hasattr(app_instance, 'tau1_display_var'):
        app_instance.tau1_display_var = tk.StringVar(value="0.00 Nm")  # Fallback
    ttk.Label(tau_frame, textvariable=app_instance.tau1_display_var, width=10, anchor="w").pack(side=tk.LEFT, padx=5)
    row_idx += 1
